...
const logos = [
  { name: "Bootstrap", src: "Bootstrap.jpg", rounded: true, border: "50%" },
...
const IconCarousel: React.FC<IconCarouselProps> = ({ reverse = false }) => {
  ...
  return (
    <div className="relative group w-screen">
      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-2 bg-slate-800 text-gray-300 text-lg px-4 py-1 rounded-md shadow-lg opacity-0 group-hover:opacity-95 transition-opacity duration-300 z-50">
    <h3 className="text-lg font-bold underline mt-2 mb-1 text-center text-gray-300">Development Stack</h3>
          ...
          >
            ...
            >
              <img
                src={logo.src}
                alt={logo.name}
                className="smallCard w-32 h-32 sm:w-20 sm:h-20 md:w-20 md:h-20 lg:w-20 lg:h-20 lg:mt-1 mx-auto border-black"
                style={{
                  border: "1.5px solid #64748b",
                }}
              />
              <h3 className="text-center mt-6 text-2xl lg:mx-0 font-outfit bg-transparent text-slate-900 font-medium tracking-normal">
                {logo.name}
              </h3>
            </div>
          </div>
